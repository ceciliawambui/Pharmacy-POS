...
$id=$_GET['iv'];
include('../connect.php');
$resultaz = $link->prepare("SELECT * FROM purchases WHERE invoice_number= :xzxz");
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<?php
				$id=$_GET['iv'];
				$result = $link->prepare("SELECT * FROM purchases_item WHERE invoice= :userid");
			...
			<td><?php
			$rrrrrrr=$row['name'];
			$resultss = $link->prepare("SELECT * FROM products WHERE product_code= :asas");
			...
			<tr>
				...
				$sdsd=$_GET['iv'];
				$resultas = $link->prepare("SELECT sum(cost) FROM purchases_item WHERE invoice= :a");
